#!/usr/bin/env Rscript

# Plot Figure 1B
#
# Dependencies can be installed with:
#    install.packages(c('magrittr', 'ggplot2', 'devtools', 'gtable'))
#    devtools::install_github('briandconnelly/ggplot2bdc')

library(magrittr)
library(ggplot2)
library(ggplot2bdc)
library(gtable)

source('formatting.R')

fig1B_data <- read.csv('../data/L05_A06_1xDelta_0xEpsilon.csv')
fig1B_data$Replicate <- as.factor(fig1B_data$Replicate)

fig1B <- ggplot(data=fig1B_data, aes(x=Time, y=CooperatorProportion)) +
    geom_hline(aes(yintercept=0.5), linetype='dotted', color='grey70') +        
    stat_summary(fun.data='mean_cl_normal', geom='ribbon', color=NA, fill='black', alpha=0.1) +
    stat_summary(fun.y='mean', geom='line', color='black') +
    scale_y_continuous(breaks=seq(from=0, to=1, by=0.25), limits=c(0,1)) +
    labs(x=label_time, y=label_cooperator_proportion) +
    theme_bdc_grey()
fig1B <- rescale_square(plot=fig1B)

g <- ggplotGrob(fig1B)                                                          
g <- gtable_add_grob(g, textGrob(expression(bold('B')), gp=gpar(col='black', fontsize=20), x=0, hjust=0, vjust=0.5), t=1, l=2)

png('../figures/Figure1B.png', width=6, height=6, units='in', res=300)   
grid.newpage()                                                                  
grid.draw(g)                                                                    
dev.off()

# Save a version with each replicate shown
ggsave(filename='../figures/Figure1B-all.png', plot=fig1B + geom_line(aes(color=Replicate)),
       width=6, height=6, units='in', dpi=300)
