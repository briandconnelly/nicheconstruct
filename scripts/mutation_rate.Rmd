...
mu_duration <- max(mu_data$Time) - min(mu_data$Time)

mu_pres <- mu_data %>%
    group_by(MutationRateSocial, MutationRateAdaptation, Source, Replicate) %>%
    summarise(Integral=sum(MeanProducerProportion)/mu_duration)
```
...
p_mu_prop <- ggplot(mu_prop, aes(x=Time, y=Mean,
    ...
    geom_line() +
    scale_color_hue(name=label_mu, guide=guide_legend(order=1)) +
