...
stress_labels <- c('1e-5'=expression(bold('0.00001')), '1e-4'='0.0001',
...
```{r analysis}
duration <- max(stress_data$Time) - min(stress_data$Time)
...
stress_pres <- stress_data %>%
    ...
    summarise(Integral=sum(MeanProducerProportion)/duration)
```
