...
# Make the plot
fig6B <- ggplot(data=fig6Bintegrals, aes(x=MigrationRate, y=Integral)) +
...
fig6B <- rescale_golden(plot=fig6B)

#ggsave_golden(plot=fig6B, filename='../figures/Figure6B.png', dpi=300)

g <- ggplotGrob(fig6B)                                                          
g <- gtable_add_grob(g, textGrob(expression(bold('B')), gp=gpar(col='black', fontsize=20), x=0, hjust=0, vjust=0.5), t=1, l=2)

png('../figures/Figure6B.png', width=6, height=3.708204, units='in', res=300)   
grid.newpage()                                                                  
grid.draw(g)                                                                    
dev.off()
