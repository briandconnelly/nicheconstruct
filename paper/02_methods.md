...
$$ n(a_{g,l}) = \sum_{i \in P} I_{a_{g,l}} (1 + a_{\gamma(i),\{1 + (l-2(\bmod L)\}} (\bmod a_{max})) $$ {#eq:numepsilon}

Here, $I_{x} (y)$ indicates whether the allelic state $y$ matches allelic state $x$ ($1$) or not ($0$), and $\gamma(j)$ is the genotype of individual $j$. Note that the selective value of an allele at the first locus is affected by the allelic state at locus $L$.
