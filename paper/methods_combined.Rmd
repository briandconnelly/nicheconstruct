...
Using a computational model, we observed the relationship between adaptation to stress and the production of public good over evolutionary timescales. The simulation software and all configurations for the experiments reported are available at TODO. Simulations were run using Python 2.7.2, NumPy 1.8.1, and NetworkX 1.9. Data analyses were performed using R 3.1.1. Model parameters and their values are listed in [Table X](https://github.com/briandconnelly/niche_hike/wiki/Table%20of%20Parameter%20Values).

# Description of metapopulation, populations, and individuals

Each simulation tracks a single metapopulation comprising $N^2$ sites, arranged as an $N×N$ bounded lattice of sites. Each site could potentially hold a population. Populations consist of individuals, where the genotype of each individual is a binary string of length $L+1$. Alleles at the first $L$ loci (bits) determine the individual’s level of adaptation to the stressful environment. We refer to these loci as “stress loci.” The allele at the $(L+1)th$ locus determines whether the individual is a producer of a public good (allele $1$) or a non-producer (allele $0$). We refer to this locus as the “production locus.”

# Individual fitness

A mutation from $0$ to $1$ at the $i$th stress locus will improve individual fitness by $w_i$ despite the allelic states of other loci (i.e., there is no epistasis). We assume that $\{w_1,w_2,w_3,...,w_L\}$ are independent and identically distributed (iid) random variables with $w_i \sim unif[w_{min}, w_{max}]$. Public good production is costly, reducing individual fitness by $c$. Thus, if the allelic state of the $i$ th locus is denoted $a_i$ (with $a_i \in \{0,1\}$), then the fitness of an individual is:
...
If the proportion of producers in a population is $p$ at the beginning of a growth cycle, then the population grows to the following size over the growth cycle:

$$
S(p) = S_{min} + (S_{max} - S_{min}) p
$$

Therefore, a population with solely non-producers reaches a size of $S_{min}$, while a population with only producers reaches a size of $S_{max}$ (with $S_{max} \gt S_{min}$). The function $S(p)$ gauges the benefit of public good production, as population size increases linearly with the proportion of producers. During population growth, competition between genotypes occurs. There are $2^{L+1}$ possible genotypes. Consider an arbitrary genotype $g$ (with $g \in \{1,2,3,...,2^{L+1}\}$). Let $n_g$ be the number of individuals with genotype $g$, and let $W_g$ be the fitness of genotype $g$ (see equation [1]). The composition of genotypes after population growth is multinomial with parameters $S(p)$ and $\{\pi_1, \pi_2, \ldots, \pi_{2^{L+1}}\}$, where
...
Let $n_{g}^{*}$ be the number of individuals with genotype $g$ after population growth and before mutation. The composition of genotypes after mutation is multinomial with parameters $S(p)$ and $\{\pi_{1}^{*}, \pi_{2}^{*}, \pi_{3}^{*}, \ldots, \pi_{L+1}^{*}\}$, where 


$$
\pi_{g'}^{*} = \frac{\sum_{g=1}^{2^{L+1}} n_{g} \tau_{g \rightarrow g'}}{\sum_{g'=1}^{2^{L+1}} \sum_{g=1}^{2^{L+1}} n_{g} \tau_{g \rightarrow g'}}
$$

Thus, $\pi_{g'}^{*}$ is the probability that an individual in the population after mutation is genotype $g'$ (such that $\sum \pi_{g'}^{*} = 1$).
...
**BDC:** Use more generic graph description if wellmixed is included?
